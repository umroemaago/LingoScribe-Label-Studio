...
export const Table = observer(
  ({
    ...
    // Safe calculation with proper null checks and mobile detection
    const calculateRightOffset = useCallback(() => {
      try {
        ...
        const wrapper = tableWrapper.current;
        if (!wrapper) return 0;
        ...
        if (!thirdChild) return 0;

        // Both elements exist, safe to access offsetWidth
        const outerWidth = secondChild.offsetWidth || 0;
        const innerWidth = thirdChild.offsetWidth || 0;
        
        return Math.max(0, outerWidth - innerWidth);
      } catch (error) {
        console.warn('Error calculating table offset:', error);
        return 0;
      }
    }, []);
    ...
    // Update right offset when component mounts and on resize
    useEffect(() => {
      const updateOffset = () => {
